...
final class GtkBaseline extends Baseline {
	...
	int fetchBaseline(Control control, int width, int height) {
		...
		try {
			...
			try {
				...
				if (getTextMethod != null) {
					...
					if (oldText == null || oldText.length() == 0) {
						...
						setTextMethod.invoke(control, new String[]{"a"});
			...
			// fetch baseline
			// special case for Link widget
			/* $if eclipse.version >= 3.1 $ */
			if (control instanceof org.eclipse.swt.widgets.Link) {
				...
				org.eclipse.swt.graphics.TextLayout textLayout = (org.eclipse.swt.graphics.TextLayout) layoutField.get(linkControl);
				Field layoutHandleField = org.eclipse.swt.graphics.TextLayout.class.getDeclaredField("layout");
			...
			if (setTextMethod != null) {
				setTextMethod.invoke(control, new String[]{""});
			}
		} catch (Throwable e) {
			e.printStackTrace();
		} 
